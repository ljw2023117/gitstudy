...
type RawPbftExtraHandleMod struct {
	pbftNode *PbftConsensusNode
	// pointer to pbft data
}

// propose request with different types
func (rphm *RawPbftExtraHandleMod) HandleinPropose() (bool, *message.Request) {
	// new blocks
...
func (rphm *RawPbftExtraHandleMod) HandleinPrePrepare(ppmsg *message.PrePrepare) bool {
...
func (rphm *RawPbftExtraHandleMod) HandleinPrepare(pmsg *message.Prepare) bool {
	fmt.Println("No operations are performed in Extra handle mod")
	return true
}

// the operation in commit.
func (rphm *RawPbftExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	if rphm.pbftNode.NodeID == rphm.pbftNode.view {
		...
		msg_send := message.MergeMessage(message.CBlockSingle, bByte)
		go networks.TcpDial(msg_send, rphm.pbftNode.ip_nodeTable[uint64(0)][0])
...
func (rphm *RawPbftExtraHandleMod) HandleReqestforOldSeq(*message.RequestOldMessage) bool {
	fmt.Println("No operations are performed in Extra handle mod")
	return true
}

// the operation for sequential requests
func (rphm *RawPbftExtraHandleMod) HandleforSequentialRequest(som *message.SendOldMessage) bool {
