...
type RawOutsideModule struct {
	pbftNode *PbftConsensusNode
}

// msgType canbe defined in message
func (rrom *RawOutsideModule) HandleMessageOutsidePBFT(msgType message.MessageType, content []byte) bool {
	switch msgType {
	case message.CRelay:
...
func (rrom *RawOutsideModule) handleRelay(content []byte) {
...
func (rrom *RawOutsideModule) handleInjectTx(content []byte) {
