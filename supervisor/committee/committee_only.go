...
type Only_CommitteeModule struct {
	csvPath      string
	dataTotalNum int
	nowDataNum   int
	batchDataNum int
...
func NewOnly_CommitteeModule(Ip_nodeTable map[uint64]map[uint64]string, Ss *signal.StopSignal, slog *supervisor_log.SupervisorLog, csvFilePath string, dataNum, batchNum int) *Only_CommitteeModule {
	return &Only_CommitteeModule{
		csvPath:      csvFilePath,
...
func data4tx(data []string, nonce uint64) (*core.Transaction, bool) {
...
func (rthm *Only_CommitteeModule) HandleOtherMessage([]byte) {
	rthm.sl.Slog.Printf("hhhhhhhhhhhhhhhhhhhh\n")
}

func (rthm *Only_CommitteeModule) txSending(txlist []*core.Transaction) {
...
func (rthm *Only_CommitteeModule) MsgSendingControl() {
...
// no operation here
func (rthm *Only_CommitteeModule) HandleBlockInfo(b *message.BlockInfoMsg) {
