...
type Tree_CommitteeModule struct {
	csvPath      string
	dataTotalNum int
	nowDataNum   int
	batchDataNum int
...
func NewTree_CommitteeModule(Ip_nodeTable map[uint64]map[uint64]string, Ss *signal.StopSignal, slog *supervisor_log.SupervisorLog, csvFilePath string, dataNum, batchNum int) *Tree_CommitteeModule {
	return &Tree_CommitteeModule{
		csvPath:      csvFilePath,
...
func data3tx(data []string, nonce uint64) (*core.Transaction, bool) {
...
func (rthm *Tree_CommitteeModule) HandleOtherMessage([]byte) {
	rthm.sl.Slog.Printf("hhhhhhhhhhhhhhhhhhhh\n")
}

func (rthm *Tree_CommitteeModule) txSending(txlist []*core.Transaction) {
...
func (rthm *Tree_CommitteeModule) MsgSendingControl() {
	...
	for {
		...
		if err != nil {
			log.Println("commit32")
			log.Panic(err)
		}
		if tx, ok := data3tx(data, uint64(rthm.nowDataNum)); ok {
...
// no operation here
func (rthm *Tree_CommitteeModule) HandleBlockInfo(b *message.BlockInfoMsg) {
