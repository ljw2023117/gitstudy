...
func BuildNewNode(nid, nnm, sid, snm, mod uint64) {
	...
	if nid == 0 {
		//var measureMod []string
		//measureMod = params.MeasureRelayMod
		//ls := new(supervisor.Supervisor)
		//log.Println(params.SupervisorAddr)
		//ls.NewSupervisor(params.SupervisorAddr, initConfig(nid, nnm, sid, snm), params.CommitteeMethod[mod], measureMod...)
		//log.Println(ls)
		worker.NewSendPbftNode(initConfig(nid, nnm, sid, snm))
		log.Println(worker.ComMod)
		time.Sleep(10000 * time.Millisecond)
		//进行传输数据
		go worker.SupervisorTxHandling()
		go worker.Propose()
